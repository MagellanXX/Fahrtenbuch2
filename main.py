...
def get_all_trips():
  ...
  c = conn.cursor()
  c.execute(
      'SELECT date, start_time, end_time, distance_km, duration_str,'
      ' avg_speed, max_speed FROM trips ORDER BY id DESC'
  )
  rows = c.fetchall()
  conn.close()
  return rows
...
class HistoryScreen(Screen):
  ...
  def __init__(self, **kwargs):
    ...
    self.root_box = BoxLayout(
        orientation='vertical', padding=14, spacing=10
    )
    with self.root_box.canvas.before:
      Color(0.07, 0.08, 0.11, 1)
      self.bg = RoundedRectangle(
          pos=self.root_box.pos, size=self.root_box.size
      )
    self.root_box.bind(
        pos=lambda i, v: setattr(self.bg, 'pos', v),
        size=lambda i, v: setattr(self.bg, 'size', v),
  ...
  def refresh_data(self):
    ...
    if not trips:
      empty_lbl = Label(
          text='Noch keine Fahrten vorhanden.',
          color=(0.5, 0.55, 0.65, 1),
          size_hint_y=None,
          height=60,
      )
